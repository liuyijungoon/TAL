...
def uncertainties(
    ...
    # temp = F.normalize(logits, p=2, dim=1)*10
    # cos = temp.softmax(dim=-1).max(dim=-1).values

    cos, _ = F.normalize(logits, p=2, dim=1).max(1)
    cos = torch.pow(cos, 1/2)
    ...
    if features is not None:
        ...
        if stats is not None:
            ...
            uncertainty[f"SIRC_cos_z"] = - sirc(
                -cos, feature_norm, feat_a, feat_b, s1_max=1
            )
            ...
            if vim_params is not None:
                ...
                uncertainty[f"SIRC_cos_res"] = -sirc( -cos, -vlogits, res_a, res_b, s1_max=1)
...
def detect_results(
    ...
    assert mode in ["ROC", "FPR@95", "AURC"]
...
def detect(
    ...
    if mode == 'ROC':
        ...
        # import pdb
        # pdb.set_trace()
        aurc, eaurc = calc_aurc_eaurc1(-scores,1-domain_labels)
        # percent
        return aurc * 1000 
